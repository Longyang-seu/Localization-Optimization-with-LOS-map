...
def get_a_map(map, q):
    ...
    for k in range(10):
        for i in range(a_num):
            a[k,i, :] = (c / (4 * np.pi * f)) ** 2 / ((map[k,:, :, 0] - q[i, 0]) ** 2 + (map[k,:, :, 1] - q[i, 1]) ** 2)
    return a

def get_rho(LOS_map, a_map, map,B_s):
    return np.sum(np.sum(map / (LOS_map * Lamb(B_s) * a_map * sig ** 2 + 1), axis=1), axis=1)
...
if __name__ == "__main__":
    ...
    # func = lambda x, y: 1 / (2 * np.pi * sig ** 2) * np.exp(-(x ** 2 + y ** 2) / (2 * sig ** 2))
    # probs = 0
    # for i in range(int(20*sig_num*sig)):
    #     for j in range(int(20*sig_num*sig)):
    #         prob, err = scipy.integrate.dblquad(func, - sig_num * sig + 0.1 * i - 0.05, - sig_num * sig + 0.1 * i + 0.05,
    ...
    # pkl.dump(LOS_map, open("all_LOS_map.pkl", "wb"))
    ...
    for n in range(10):
        ...
        for k in range(10):
                ...
                for i in U_q.difference(N_s):
                    ...
                    for j in N_s | {i}:
                        Psi_temp += Lamb(B_s) / (len(N_s) + 1) ** 2 * np.sum(
                            np.sum((a_map[k,j, :, :] * LOS_map[k,j, :, :] * all_map[k,:, :, 2]), axis=1))
                    Psi.append(Psi_temp)
                    ...
                    Upsilon_temp3 = 0
                    for j in N_s | {i}:
                        Upsilon_temp1 += Lamb(B_s) / (len(N_s) + 1) ** 2 * np.sum(np.sum((a_map[k,j, :, :] * LOS_map[k,j, :,
                                                                                                      :] * all_map[k,:, :,
                                                                                                           2] * np.cos(
                            get_phi(all_map[k,:, :, 0:2], q[j, :])) ** 2), axis=1))
                        Upsilon_temp2 += Lamb(B_s) / (len(N_s) + 1) ** 2 * np.sum(np.sum((a_map[k,j, :, :] * LOS_map[k,j, :,
                                                                                                      :] * all_map[k,:, :,
                                                                                                           2] * np.sin(
                            get_phi(all_map[k,:, :, 0:2], q[j, :])) ** 2), axis=1))
                        Upsilon_temp3 += Lamb(B_s) / (len(N_s) + 1) ** 2 * np.sum(np.sum((a_map[k,j, :, :] * LOS_map[k,j, :,
                                                                                                      :] * all_map[k,:, :,
                                                                                                           2] * np.cos(
                            get_phi(all_map[k,:, :, 0:2], q[j, :])) * np.sin(get_phi(all_map[k,:, :, 0:2], q[j, :]))), axis=1))
                    Upsilon.append(Upsilon_temp1 * Upsilon_temp2 - Upsilon_temp3 ** 2)
                    index.append(i)
                SPEB_temp = (np.array(Psi) + 2 * sig ** (-2)) / (
                            np.array(Upsilon) + sig ** (-2) * np.array(Psi) + sig ** (-4))
            ...
            for i in range(fix_num):
                ...
                for j in range(i):
                    Psi_temp += Lamb(B_s) / (i + 1) ** 2 * np.sum(
                        np.sum((a_map[k,int(dis_q[j, 1]), :, :] * LOS_map[k,int(dis_q[j, 1]), :, :] * all_map[k,:, :, 2]),
                               axis=1))
                    Upsilon_temp1 += Lamb(B_s) / (i + 1) ** 2 * np.sum(
                        np.sum((a_map[k,int(dis_q[j, 1]), :, :] * LOS_map[k,int(dis_q[j, 1]), :, :] * all_map[k,:, :, 2] * np.cos(
                            get_phi(all_map[k,:, :, 0:2], q[int(dis_q[j, 1]), :])) ** 2),
                               axis=1))
                    Upsilon_temp2 += Lamb(B_s) / (i + 1) ** 2 * np.sum(
                        np.sum((a_map[k,int(dis_q[j, 1]), :, :] * LOS_map[k,int(dis_q[j, 1]), :, :] * all_map[k,:, :, 2] * np.sin(
                            get_phi(all_map[k,:, :, 0:2], q[int(dis_q[j, 1]), :])) ** 2),
                               axis=1))
                    Upsilon_temp3 += Lamb(B_s) / (i + 1) ** 2 * np.sum(
                        np.sum((a_map[k,int(dis_q[j, 1]), :, :] * LOS_map[k,int(dis_q[j, 1]), :, :] * all_map[k,:, :,
                                                                                                  2] * np.cos(
                            get_phi(all_map[k,:, :, 0:2], q[int(dis_q[j, 1]), :])) * np.sin(
                            get_phi(all_map[k,:, :, 0:2], q[int(dis_q[j, 1]), :]))), axis=1))
                Psi.append(Psi_temp)
                Upsilon.append(Upsilon_temp1 * Upsilon_temp2 - Upsilon_temp3 ** 2)
                SPEB_temp = np.array(
                    (np.array(Psi) + 2 * sig ** (-2)) / (np.array(Upsilon) + sig ** (-2) * np.array(Psi) + sig ** (-4)))
                SPEB_md.append(SPEB_temp[i])
            avg_SPEB_md[0,n,k]=2 * sig ** 2
            for i in range(fix_num):
            ...
            for j in range(a_num):
                Psi += Lamb(B_s) / (a_num) ** 2 * np.sum(np.sum((a_map[k,j, :, :] * LOS_map[k,j, :, :] * all_map[k,:, :, 2]), axis=1))
                Upsilon_temp1 += Lamb(B_s) / (a_num) ** 2 * np.sum(np.sum((a_map[k,j, :, :] * LOS_map[k,j, :,
                                                                                                  :] * all_map[k,:, :,
                                                                                                       2] * np.cos(
                        get_phi(all_map[k,:, :, 0:2], q[j, :])) ** 2), axis=1))
                Upsilon_temp2 += Lamb(B_s) / (a_num) ** 2 * np.sum(np.sum((a_map[k,j, :, :] * LOS_map[k,j, :,
                                                                                                  :] * all_map[k,:, :,
                                                                                                       2] * np.sin(
                        get_phi(all_map[k,:, :, 0:2], q[j, :])) ** 2), axis=1))
                Upsilon_temp3 += Lamb(B_s) / (a_num) ** 2 * np.sum(np.sum((a_map[k,j, :, :] * LOS_map[k,j, :,
                                                                                                  :] * all_map[k,:, :,
                                                                                                       2] * np.cos(
                        get_phi(all_map[k,:, :, 0:2], q[j, :])) * np.sin(get_phi(all_map[k,:, :, 0:2], q[j, :]))), axis=1))
            Upsilon=(Upsilon_temp1 * Upsilon_temp2 - Upsilon_temp3 ** 2)
            avg_allSPEB[n,k]=(Psi + 2 * sig ** (-2)) / (Upsilon + sig ** (-2) * Psi + sig ** (-4))
            ...
            while i < a_num and ind < a_num and len(index)<3:
                ...
                while True:
                    if ind >= a_num:
                        break
                    if LOS_map[k,int(dis_q[ind, 1]), int(10 * sig_num * sig), int(10 * sig_num * sig)] != 0:
                        index.append(ind)
                        ind += 1
                        break
                    else:
    ...
    pkl.dump(avg_SPEB, open("avg_SPEB.pkl", "wb"))
    pkl.dump(avg_SPEB_md, open("avg_SPEB_md.pkl", "wb"))
    pkl.dump(avg_allSPEB, open("avg_allSPEB.pkl", "wb"))
    pkl.dump(avg_SPEB_ep, open("avg_SPEB_ep.pkl", "wb"))
